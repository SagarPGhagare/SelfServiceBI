  ...
  $(document).ready(function(){
	  ...
	  if(msg.result=="success")
	  ...
	  else if(msg.errorMessage)
	    {
			...
			$("#errorMsgTxt").html(msg.errorMessage);
		  }
	  else
		  {
			$("#successMessageDiv").css("display","none");
...
<div id="errorMessageDiv" style="display:none" class="msg-alert alert alert-danger" role="alert">
  ...
  <form id="updateForm" role="form" action="createStorage" name="createStoragePlugin" method="POST">
    <input type="hidden" name="storageName" value="${name}" />
    <div class="form-group">
      <textarea class="form-control" name="jsonString" id="config" rows="20" cols="50" name="config" style="font-family: Courier;">
 ...
 <%--  <h3>${message}</h3>
  ...
  <script>
    $.get("/http://localhost:8080/storage/${name}.json", function(data) {
      $("#config").val(JSON.stringify(data.config, null, 2));
    });
    $("#enabled").click(function() {
      $.get("/storage/${name}/enable/true", function(data) {
    ...
    function deleteFunction() {
      ...
      if (temp == true) {
        $.get("/storage/${name}/delete", function(data) {
